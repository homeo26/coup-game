...
function decide(g: GameState): Move | null {
  ...
  if (g.phase === 'action' && g.players[g.turn].id === myId) {
    if (me.coins >= 7) {
      const tgt = g.players.find((p) => p.id !== myId && isAlive(p));
      if (tgt) return { type: 'declare', action: 'coup', target: tgt.id };
    }
    return { type: 'declare', action: 'income' };
  }
  ...
  if (g.phase === 'exchange' && g.pending?.actor === myId) {
    ...
    return { type: 'exchange_keep', keep: Array.from({ length: n }, (_, i) => i) };
  }
